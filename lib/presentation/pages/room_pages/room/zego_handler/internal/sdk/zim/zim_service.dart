...
class ZIMService {
  ...
  final roomStateChangedStreamCtrl =
      StreamController<ZIMServiceRoomStateChangedEvent>.broadcast();
  final receiveRoomCustomSignalingStreamCtrl =
  ...
  final outgoingUserRequestAcceptedStreamCtrl =
      StreamController<OutgoingUserRequestAcceptedEvent>.broadcast();
  final outgoingUserRequestRejectedStreamCtrl =
      StreamController<OutgoingUserRequestRejectedEvent>.broadcast();
  final incomingUserRequestTimeoutStreamCtrl =
      StreamController<IncomingUserRequestTimeoutEvent>.broadcast();
  final outgoingUserRequestTimeoutStreamCtrl =
      StreamController<OutgoingUserRequestTimeoutEvent>.broadcast();
  final userRequestEndStreamCtrl =
      StreamController<UserRequestEndEvent>.broadcast();
  final userRequestStateChangeStreamCtrl =
      StreamController<UserRequestStateChangeEvent>.broadcast();

  final roomAttributeUpdateStreamCtrl =
      StreamController<ZIMServiceRoomAttributeUpdateEvent>.broadcast();
  final roomAttributeUpdateStreamCtrl2 =
      StreamController<RoomAttributesUpdatedEvent>.broadcast();
  final roomAttributeBatchUpdatedStreamCtrl =
      StreamController<ZIMServiceRoomAttributeBatchUpdatedEvent>.broadcast();
  final sendRoomRequestStreamCtrl =
      StreamController<SendRoomRequestEvent>.broadcast();
  final acceptIncomingRoomRequestStreamCtrl =
      StreamController<AcceptIncomingRoomRequestEvent>.broadcast();
  final rejectIncomingRoomRequestStreamCtrl =
      StreamController<RejectIncomingRoomRequestEvent>.broadcast();
  final cancelRoomRequestStreamCtrl =
      StreamController<CancelRoomRequestEvent>.broadcast();

  final onInComingRoomRequestStreamCtrl =
      StreamController<OnInComingRoomRequestReceivedEvent>.broadcast();
  final onOutgoingRoomRequestAcceptedStreamCtrl =
      StreamController<OnOutgoingRoomRequestAcceptedEvent>.broadcast();
  final onOutgoingRoomRequestRejectedStreamCtrl =
      StreamController<OnOutgoingRoomRequestRejectedEvent>.broadcast();
  final onInComingRoomRequestCancelledStreamCtrl =
  ...
  Future<void> connectUser(String userID, String userName,
      {String? token, bool isOfflineLogin = false}) async {
  ...
  Future<ZegoRoomLoginResult> _loginRoom({
    ...
    await ZIM.getInstance()!.joinRoom(roomID).then((zimResult) {
      ...
      debugPrint(
          'zim service, loginRoom(join), room id:$currentRoomID, error:$error');
  ...
  Future<ZegoRoomLoginResult> loginRoom(
    ...
    await ZIM
      ...
      if (error is PlatformException &&
        ...
        debugPrint(
            'zim service, loginRoom, room id:$currentRoomID, error:$error');
  ...
  Future<ZIMRoomLeftResult> logoutRoom() async {
    ...
    if (currentRoomID != null) {
      ...
      final ret =
          await ZIM.getInstance()!.leaveRoom(targetRoomID).then((value) {
        debugPrint('zim service, logoutRoom, room id:$targetRoomID');
        return value;
      }).catchError((error) {
        debugPrint(
            'zim service, logoutRoom, room id:$targetRoomID, error:$error');
        return ZIMRoomLeftResult(roomID: currentRoomID ?? '');
      });
  ...
  void onConnectionStateChanged(
    ...
    connectionStateStreamCtrl
        .add(ZIMServiceConnectionStateChangedEvent(state, event, extendedData));
  }

  void onRoomStateChanged(_, ZIMRoomState state, ZIMRoomEvent event,
      Map extendedData, String roomID) {
    currentRoomState = state;

    debugPrint(
        'zim service, onRoomStateChanged, room id:$roomID, state:$state');
    roomStateChangedStreamCtrl.add(
        ZIMServiceRoomStateChangedEvent(roomID, state, event, extendedData));
  }

  void cancelInvitation(
      {required String invitationID, required List<String> invitees}) {}
}
